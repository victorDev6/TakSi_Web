$(document).ready(function () {
        ...
        querychoferes.forEach(function (docnumchoferes) {
          ...
          if (docnumchoferes.exists) {
            document.getElementById(
              "inputNombre" + conuntvariablenumcaja
            ).disabled = true;
            document.getElementById(
              "inputApellido" + conuntvariablenumcaja
            ).disabled = true;
            document.getElementById(
              "inputTelefono" + conuntvariablenumcaja
            ).disabled = true;
            document.getElementById(
              "inputEmail" + conuntvariablenumcaja
            ).disabled = true;
            ...
            //extrallendo valor
            var nombreChofer = (document.getElementById(
              "inputNombre" + conuntvariablenumcaja
            ).value = cajaNombre);
            var apellidoChofer = (document.getElementById(
              "inputApellido" + conuntvariablenumcaja
            ).value = cajaApellidos);
            var telefonoChofer = (document.getElementById(
              "inputTelefono" + conuntvariablenumcaja
            ).value = cajaTelefono);
            var emailChofer = (document.getElementById(
              "inputEmail" + conuntvariablenumcaja
            ).value = cajaCorreo);
  ...
  $(".acepataraltaChofer").click(function () {
    ...
    //extrallendo valor
    var nombreChofer = document.getElementById("inputNombre" + nuermoChofer)
      .value;
    var apellidoChofer = document.getElementById("inputApellido" + nuermoChofer)
      .value;
    var telefonoChofer = document.getElementById("inputTelefono" + nuermoChofer)
      .value;
    var emailChofer = document.getElementById("inputEmail" + nuermoChofer)
      .value;
    var passwordChofer = document.getElementById("inputPass" + nuermoChofer)
      .value;
    ...
    db.collection("choferes")
      ...
      .then(function (querySnapshot) {
        ...
        if (querySnapshot.size != 0) {
          ...
          db.collection("choferes")
            .get()
            .then(function (queryColl) {
              db.collection("choferes")
                .get()
                .then(function (queryIdentificador) {
                  queryIdentificador.forEach(function (docidentificador) {
                    if (
                      numerosaleatorios1.toString() ==
                      docidentificador.data().identificador
                    ) {
                      console.log(
                        "!Si esta en la BD! :" + numerosaleatorios1.toString()
                      );
                      ...
                      console.log(
                        "!No esta en la BD!: " + numerosaleatorios1.toString()
                      );
    ...
    firebase
      .auth()
      .createUserWithEmailAndPassword(emailcompleto, passwordChofer)
      .then(function (result) {
        ...
        /*console.log("Apellido: " + apellidoChofer);
        console.log("EmailP: " + emailP);
        console.log("Identificador: " + numerosaleatorios1);
        console.log("Telefono: " + telefonoChofer);
        console.log("Placa: " + placaAGR);
        console.log("Nombre Chofer: " + nombreChofer);*/
      })
  ...
  function SubirImgChofer(ApeCh, emailP, emailMin, numAle, nomCh, placaT, tel) {
  ...
  function GuardarDatosChofer(
    ...
    db.collection("choferes")
      ...
      .then(function (docRef) {
        ...
        /**En esta parte no es necesario que hagas consulta para obtener el id
			simplemente usa el parametro que tiene function "docRef" quedaria
			como docRef.id simplemente con eso obtienes el id del registro
			que recien guardaste */
        db.collection("taxis")
          .where("placa", "==", placaT)
          .where("correo", "==", emailP)
          .onSnapshot(function (queryidDoc) {
            var idDocTaxi;
        ...
        /*Pienso que aca debe ser dinamico, osea que pasaria si nos 
			vamos a otra ciudad?*/
        db.collection("Comitán de Domínguez")
          .doc(numAle)
          .set({
            dueño: emailP,
            estado: "offline",
